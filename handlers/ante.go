...
func embeddedAnteHandler(ctx sdk.Context, tx types.EmbeddedTx, am auth.AccountMapper) (sdk.Context, sdk.Result, bool) {
	...
	for i, sig := range tx.Signatures {
		...
		// attempt to get the account and sequence number of the signer
		acc := am.GetAccount(ctx, signer.Bytes())
		if acc == nil {
			return ctx, sdk.ErrUnknownAddress("No account with that address in state").Result(), true
		}
